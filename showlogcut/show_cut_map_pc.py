...
def summry(shows,record,lock):
    ...
    for v2 in shows:
	    ...
	    if int(v2['channel_source'])>>30 == 1 and not re.match(r'.*(php|java|javascript|perl|python|bingbot|Soso|Yahoo!|Sogou*.spider|Googlebot|Baiduspider|Baiduspider|EasouSpider).*',v2['user_agent'],re.M|re.I):	
		for tp in tp1:
    ...
    for key,value in middle.items():
        try:
            if int(key) < 10:
                f1 = open('/home/brdwork/antispider/data/showsmap/range-000'+str(key),'a+')
            elif int(key) < 100:
                f1 = open('/home/brdwork/antispider/data/showsmap/range-00'+str(key),'a+')
            elif int(key) <1000:
                f1 = open('/home/brdwork/antispider/data/showsmap/range-0'+str(key),'a+')
            else:
                f1 = open('/home/brdwork/antispider/data/showsmap/range-'+str(key),'a+')
            for value1 in value:
                f1.write(value1+"\n")
	    f1.flush()
            f1.close()
